...
export const AuthLayout: React.FC<AuthLayoutProps> = ({
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center p-4 sm:p-6 lg:p-10 relative z-10 bg-[#F8FAF8]">
        ...
        <div className="lg:col-span-5 space-y-6 text-left">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 border ${accentBgClass} ${accentBorderClass}`}>
                <RoleIcon className={`w-4 h-4 ${accentTextClass}`} />
                <span className={`text-[11px] font-bold uppercase tracking-wider ${accentTextClass}`}>
                  {roleName}
                </span>
              </div>
            </div>
          ...
          {imageUrl && (
            <div className="w-full h-40 sm:h-48 lg:h-56 rounded-2xl overflow-hidden border border-gray-200 shadow-xs relative">
              <img src={imageUrl} alt={imageAlt || roleName} className="w-full h-full object-cover" />
          ...
          <div className="space-y-2.5 pt-2">
            {benefits.map((b, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white border border-gray-200 shadow-2xs">
                <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${accentTextClass}`} />
                <div>
